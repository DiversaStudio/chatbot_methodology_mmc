...
def load_sami(responses_path=None, meal_path=None) -> SamiData:
    ...
    # Underscore-anchored: bare "P1" would also match "P11"-style names.
    # P9_ (summary format) was deleted with the category taxonomy on 2026-08-03.
    failed = [c for c in checks if not c[1] and c[0].startswith(("P1_", "P6_"))]
    if failed:
