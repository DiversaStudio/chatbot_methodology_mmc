...
The run prints numbered, timed stages (e.g. `[5/9] sentiment over 2991
messages …`), so a slow stage is visibly working rather than hung.
...
| `tone labels` | `validation/tone_labels_analyst.csv` exists (skipped, and reported `OK`, under `--skip-nlp`). | A full run validates the sentiment model against these human labels. Restore the file, or run with `--skip-nlp`. |
