...
| `tone labels` | `validation/tone_labels_analyst.csv` exists (skipped, and reported `OK`, under `--skip-nlp`). See [`DATA_SOURCES.md`](DATA_SOURCES.md) for what this file is, its columns, and why it is committed rather than dropped into `datasets/`. | A full run validates the sentiment model against these human labels. Restore the file, or run with `--skip-nlp`. |
